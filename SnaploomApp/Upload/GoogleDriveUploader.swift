...
/// Files are uploaded to a "Snaploom" folder in the user's Drive, kept private (not shared).
final class GoogleDriveUploader: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    // Fork maintainers must supply their own native OAuth client in Info.plist.
    private let clientID = Bundle.main.object(forInfoDictionaryKey: "GoogleOAuthClientID") as? String ?? ""
    static var isConfigured: Bool { !shared.clientID.isEmpty }
    ...
    private var snaploomFolderID: String?
    ...
    func signOut() {
        ...
        snaploomFolderID = nil
    }

    /// Progress callback: percentage 0.0–1.0
    var onProgress: ((Double) -> Void)?

    /// Upload a file (image or video) to the Snaploom folder.
    func upload(data: Data, filename: String, mimeType: String, completion: @escaping (Result<String, Error>) -> Void) {
        ensureValidToken { [weak self] success in
            ...
            self.ensureSnaploomFolder { result in
                switch result {
    ...
    private func ensureSnaploomFolder(completion: @escaping (Result<String, Error>) -> Void) {
        if let id = snaploomFolderID { completion(.success(id)); return }
        ...
        // Search for existing Snaploom folder
        let query = "name='Snaploom' and mimeType='application/vnd.google-apps.folder' and trashed=false"
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let existing = files.first, let id = existing["id"] as? String {
                self.snaploomFolderID = id
                DispatchQueue.main.async { completion(.success(id)) }
            } else {
                self.createSnaploomFolder(token: token, completion: completion)
            }
        }.resume()
    }

    private func createSnaploomFolder(token: String, completion: @escaping (Result<String, Error>) -> Void) {
    ...
    private func uploadFileWithRetry(data fileData: Data, filename: String, mimeType: String, folderID: String, attempt: Int, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        let tmpFile = FileManager.default.temporaryDirectory.appendingPathComponent("snaploom_upload_\(UUID().uuidString).tmp")
    ...
    private var tokenFileURL: URL {
        ...
        let dir = appSupport.appendingPathComponent("io.github.xiaoouwaou.snaploom")
