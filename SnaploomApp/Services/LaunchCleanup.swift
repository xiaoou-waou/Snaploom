...
/// Sweeps Snaploom-owned files from `NSTemporaryDirectory()` that match
...
///
/// Preserves:
///   - `snaploom-clipboard.png` and `snaploom-clipboard-recording.*`
...
private struct TmpFileCleaner: LaunchCleaner {
    ...
    /// Filename prefixes we know are ours. The `snaploom-clipboard-`
    /// prefix here is intentional to catch the *legacy* UUID-named form
    /// from pre-fix builds; the current single-file `snaploom-clipboard.png`
    ...
    private func isStale(name: String) -> Bool {
        ...
        if name == "snaploom-clipboard.png" { return false }
        if name.hasPrefix("snaploom-clipboard-recording.") { return false }
...
/// Sweeps everything in `tmp/snaploom-share/` older than 5 minutes.
...
/// Sweeps the legacy `/tmp/snaploom-clipboard/` folder used by older builds.
...
private struct LegacyClipboardTmpDirectoryCleaner: LaunchCleaner {
    ...
    func sweep() -> DirectorySweeper.Result {
        return DirectorySweeper.sweep(
            directory: FileManager.default.temporaryDirectory.appendingPathComponent("snaploom-clipboard"),
            olderThan: ttl,
            shouldDelete: { _ in true }
        )
    }
