...
///
/// Snaploom stores all preferences in `UserDefaults.standard`, which lives inside the
...
/// A hand-maintained allowlist of every Snaploom key is the opposite failure: every new feature
...
///   - Snaploom's own keys are author-written `camelCase` / `snake_case`, starting lowercase.
...
/// So a NEW Snaploom feature is exported with zero maintenance, while OS junk is excluded by
...
///   - `excludedKeys` lists the handful of Snaploom-owned keys that DO match the shape rule but
///     are machine-specific (bookmarks, geometry, device UIDs) or migration bookkeeping.
enum SettingsPortability {

    // MARK: - Envelope

    static let fileType = "Snaploom-settings"
    static let schemaVersion = 1

    /// A dated, human-friendly default filename, e.g. `Snaploom-settings-2026-07-08.json`.
    ...
    static func suggestedExportFilename() -> String {
        ...
        return "Snaploom-settings-\(fmt.string(from: Date())).json"
    }
    ...
    /// Apple doesn't churn these. Anything starting with one of these is not a Snaploom setting.
    ...
    /// injected by macOS, not Snaploom. Kept small; add only when a real export surfaces one.
    ...
    /// A key "looks like a Snaploom setting" if it's author-written: starts with a lowercase
    ...
    /// Snaploom-owned settings whose names don't fit the lowercase shape rule (e.g. the Sparkle
    /// pref, which Snaploom deliberately reuses). Explicitly allowed so they still export.
    ...
    static func isPortable(_ key: String) -> Bool {
        // Machine-specific / migration Snaploom keys that would otherwise pass the shape rule.
        ...
        // Explicitly-allowed Snaploom keys that don't match the lowercase shape rule.
        ...
        // Finally: only export things shaped like a Snaploom-authored setting.
    ...
    private static let dataTag = "__snaploomData__"
    ...
    enum ImportError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .notJSON, .wrongFileType:
                return L("This file is not a valid Snaploom settings file.")
            case .newerSchema:
                return L("This settings file was made by a newer version of Snaploom. Please update Snaploom first.")
            case .missingSettings:
