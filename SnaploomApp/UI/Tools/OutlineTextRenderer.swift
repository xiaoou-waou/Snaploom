...
/// stroke never fires:
///   - `.snaploomOutlineColor`  (NSColor)  — the outline color
///   - `.snaploomOutlineWidth`  (CGFloat)  — outline thickness in points (optional;
...
extension NSAttributedString.Key {
    static let snaploomOutlineColor = NSAttributedString.Key("snaploomOutlineColor")
    static let snaploomOutlineWidth = NSAttributedString.Key("snaploomOutlineWidth")
}
...
enum OutlineTextRenderer {
    ...
    static func applyOutline(_ color: NSColor?, to s: NSMutableAttributedString,
        ...
        if let color {
            s.addAttribute(.snaploomOutlineColor, value: color, range: r)
            if let width { s.addAttribute(.snaploomOutlineWidth, value: width, range: r) }
            else { s.removeAttribute(.snaploomOutlineWidth, range: r) }
        } else {
            s.removeAttribute(.snaploomOutlineColor, range: r)
            s.removeAttribute(.snaploomOutlineWidth, range: r)
        }
    }
    ...
    static func normalizeLegacyStroke(_ s: NSMutableAttributedString) {
        ...
        for (range, color) in conversions {
            if s.attribute(.snaploomOutlineColor, at: range.location, effectiveRange: nil) == nil {
                s.addAttribute(.snaploomOutlineColor, value: color, range: range)
            }
        }
    }
...
/// NSLayoutManager that renders `.snaploomOutlineColor` as an outline OUTSIDE the
...
final class OutlineTextLayoutManager: NSLayoutManager {
    ...
    override func drawGlyphs(forGlyphRange glyphsToShow: NSRange, at origin: NSPoint) {
        ...
        storage.enumerateAttribute(.snaploomOutlineColor, in: charRange) { value, subCharRange, _ in
            ...
            if let color = value as? NSColor {
                ...
                let explicit = storage.attribute(.snaploomOutlineWidth, at: subCharRange.location,
