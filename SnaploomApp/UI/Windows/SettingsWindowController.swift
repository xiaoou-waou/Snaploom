...
class SettingsWindowController: NSWindowController, NSToolbarDelegate, NSWindowDelegate {
    ...
    private func setupUI() {
        ...
        // Footer labels
        let madeBy = NSTextField(labelWithString: "Maintained by xiaoou")
        ...
        let linkBtn = NSButton(title: "github.com/xiaoou-waou/Snaploom", target: self, action: #selector(openGitHub))
        linkBtn.bezelStyle = .inline
        linkBtn.isBordered = false
        linkBtn.font = NSFont.systemFont(ofSize: 11)
        linkBtn.attributedTitle = NSAttributedString(string: "github.com/xiaoou-waou/Snaploom", attributes: [
    ...
    private func makeGeneralTabView() -> NSView {
        ...
        let hideNote = NSTextField(wrappingLabelWithString: L("Hotkeys still work. To show the icon again, re-launch Snaploom."))
        ...
        let urlSchemeCheckbox = NSButton(checkboxWithTitle: L("Enable snaploom:// URL scheme"), target: self, action: #selector(urlSchemeChanged(_:)))
        ...
        let backupNote = NSTextField(wrappingLabelWithString: L("Export your preferences to a file to move them to another Mac or a clean install. Upload credentials, your save folder, and screenshot history are not included. Settings are stored inside Snaploom's app container."))
    ...
    private func performImport(_ data: Data) {
        ...
        alert.informativeText = String(format: L("%d settings were applied. Relaunch Snaploom to apply all changes."), result.appliedCount)
    ...
    private func makeCaptureTabView() -> NSView {
        ...
        let menuOrderNote = NSTextField(wrappingLabelWithString: L("Choose the order of capture actions in the Snaploom menu bar menu."))
    ...
    private func makeUploadsTabView() -> NSView {
        ...
        providerPopup.item(at: 1)?.isEnabled = GoogleDriveUploader.isConfigured
        var currentProvider = UserDefaults.standard.string(forKey: "uploadProvider") ?? "imgbb"
        if currentProvider == "gdrive" && !GoogleDriveUploader.isConfigured {
            currentProvider = "imgbb"
            UserDefaults.standard.set(currentProvider, forKey: "uploadProvider")
        }
        ...
        gdriveSignInBtn.toolTip = GoogleDriveUploader.isConfigured
            ? nil
            : "Set GoogleOAuthClientID and its callback URL scheme in Info.plist to enable Google Drive."
        updateGDriveButton()
        ...
        let gdriveNote = NSTextField(wrappingLabelWithString: L("Files are uploaded to a \"Snaploom\" folder in your Google Drive. Everything stays private — nothing is shared publicly."))
        ...
        imgbbKeyField.placeholderString = "Required"
        ...
        let imgbbNote = NSTextField(wrappingLabelWithString: "Add your own free API key from imgbb.com/api. Images only; video is not supported.")
    ...
    private func makeAboutTabView() -> NSView {
        ...
        #if OFFLINE
        let offlineNote = NSTextField(wrappingLabelWithString: L("Offline build: upload and cloud storage integrations are removed. Update checks may still connect to Snaploom's update server. Screenshots and recordings stay local unless you share or save them yourself."))
    ...
    @objc private func copyScreenInfo() {
        if #available(macOS 14.0, *) {
            Task { @MainActor in
                ...
                let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "?"
                let build = Bundle.main.infoDictionary?["CFBundleVersion"] as? String ?? "?"
                lines.append("Snaploom \(version) (\(build))")
    ...
    private func updateGDriveStatus() {
        if !GoogleDriveUploader.isConfigured {
            gdriveStatusLabel?.stringValue = "Not configured"
            gdriveStatusLabel?.textColor = .secondaryLabelColor
        } else if GoogleDriveUploader.shared.isSignedIn {
    ...
    private func updateGDriveButton() {
        gdriveSignInBtn?.isEnabled = GoogleDriveUploader.isConfigured
    ...
    @objc private func s3TestTapped(_ sender: NSButton) {
        ...
        let testData = Data("Snaploom connection test".utf8)
        let testKey = ".snaploom_test_\(UUID().uuidString.prefix(8)).txt"
    ...
    @objc private func openGitHub() {
        if let url = URL(string: "https://github.com/xiaoou-waou/Snaploom") { NSWorkspace.shared.open(url) }
    }
    ...
    fileprivate func showURLSchemeInfoPopover(near sourceView: NSView) {
        ...
        let commands: [(String, String)] = [
            ("snaploom://capture",             L("Start area capture")),
            ("snaploom://capture-fullscreen",  L("Capture the full screen")),
            ("snaploom://capture-last",        L("Re-capture the last selected area")),
            ("snaploom://quick-capture",       L("Quick capture (uses your Enter action)")),
            ("snaploom://ocr",                 L("Capture area and read text/QR codes")),
            ("snaploom://ocr-translate?target=zh-CN", L("Capture, translate, and overlay the text on the image")),
            ("snaploom://record",              L("Start area recording")),
            ("snaploom://record-fullscreen",   L("Start full-screen recording")),
            ("snaploom://stop-recording",      L("Stop the current recording")),
            ("snaploom://scroll-capture",      L("Start scroll capture")),
            ("snaploom://history",             L("Open the recent captures overlay")),
            ("snaploom://settings",            L("Open this settings window")),
            ("snaploom://open?file=/path.png", L("Open an image file in the editor")),
            ("snaploom://edit?id=<id>",        L("Open a history entry in the editor (keeps annotations editable)")),
        ]
        ...
        let subtitle = NSTextField(wrappingLabelWithString: L("Trigger Snaploom from Raycast, Alfred, Shortcuts, or any tool that opens URLs."))
