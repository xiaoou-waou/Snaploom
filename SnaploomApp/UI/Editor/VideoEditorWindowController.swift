...
final class VideoEditorWindowController: NSObject, NSWindowDelegate {
    ...
    private func show(url: URL, deleteOnClose: Bool = true) {
        ...
        win.title = "\(url.deletingPathExtension().lastPathComponent) — \(L("Snaploom Video Editor"))"
...
private final class VideoEditorView: NSView {
    ...
    private func reencodeExport(asset: AVAsset, timeRange: CMTimeRange, outputURL: URL, progress: ((Double) -> Void)? = nil, completion: @escaping (Bool) -> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                let videoQueue = DispatchQueue(label: "Snaploom.export.video")
                let audioQueue = DispatchQueue(label: "Snaploom.export.audio")
    ...
    private func uploadVideo() {
        ...
        if !needsExport {
            ...
            guard let asset = asset else { return }
            let tmpURL = FileManager.default.temporaryDirectory.appendingPathComponent("snaploom_upload_\(UUID().uuidString).mp4")
