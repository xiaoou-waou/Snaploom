...
class OverlayView: NSView {
    ...
    func startScrollCaptureMode() {
        ...
        // Escape key monitor — global catches when another app has focus; local when Snaploom has focus.
    ...
    private func finishSelection() {
        ...
        // Auto-trigger OCR + translate + in-place overlay (snaploom://ocr-translate).
    ...
    private func showCameraPermissionAlert() {
        ...
        alert.informativeText = L("Snaploom needs camera permission for the webcam overlay. Open System Settings to grant access.")
    ...
    func applyGlyphStrokeToLiveTextView() {
        ...
        if enabled {
            tv.typingAttributes[.snaploomOutlineColor] = textEditor.glyphStrokeColor
        } else {
            tv.typingAttributes.removeValue(forKey: .snaploomOutlineColor)
        }
    ...
    private func showMicPermissionAlert() {
        ...
        alert.informativeText =
            L("Snaploom needs microphone permission to record voice audio. Open System Settings to grant access.")
        alert.alertStyle = .warning
        alert.addButton(withTitle: L("Open Settings"))
        alert.addButton(withTitle: L("Cancel"))
        let response = alert.runModal()
    ...
    private static let annotationPasteboardType = NSPasteboard.PasteboardType("io.github.xiaoouwaou.snaploom.annotations")
