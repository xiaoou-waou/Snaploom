...
class OverlayWindowController {
    ...
    private func setupWindow(screen: NSScreen) {
        ...
        // Snaploom app — no NSApp.activate, no focus dance with the previous app.
    ...
    /// after the user makes a selection (snaploom://ocr-translate). `targetLang`
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                guard let result = request.results?.first else {
                    throw NSError(domain: "Snaploom", code: 1)
                }
                ...
                guard let filter = CIFilter(name: "CIBlendWithMask") else {
                    throw NSError(domain: "Snaploom", code: 2)
                }
                ...
                guard let outputCIImage = filter.outputImage else {
                    throw NSError(domain: "Snaploom", code: 3)
                }
                ...
                else { throw NSError(domain: "Snaploom", code: 4) }
