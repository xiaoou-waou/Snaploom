...
final class RecordingEngine: NSObject {
    ...
    /// with the main actor.
    private let recordingQueue = DispatchQueue(label: "Snaploom.recording")
    ...
    private func beginCapture(rect: NSRect) async {
        do {
            ...
            // Exclude specific Snaploom UI chrome windows (selection border, HUD)
            ...
            if #available(macOS 13.0, *) {
                ...
                config.excludesCurrentProcessAudio = true  // don't capture Snaploom's own sounds
            }
