...
private let timingLog = OSLog(subsystem: "io.github.xiaoouwaou.snaploom", category: "capture-timing")
...
private var snaploomSignalLogFd: Int32 = -1

/// Async-signal-safe SIGTERM handler. Writes a one-line diagnostic to the
/// pre-opened `snaploomSignalLogFd`, then resets the handler to default and
...
private let sigtermHandler: @convention(c) (Int32) -> Void = { _ in
    guard snaploomSignalLogFd >= 0 else {
        signal(SIGTERM, SIG_DFL)
        return
    }
    // Only async-signal-safe operations below.
    let msg: StaticString = "SIGTERM received — likely Jetsam memory-pressure kill\n"
    _ = write(snaploomSignalLogFd, msg.utf8Start, msg.utf8CodeUnitCount)
    _ = close(snaploomSignalLogFd)
    snaploomSignalLogFd = -1
...
private final class CaptureTimingTrace: @unchecked Sendable {
    ...
    func report(finalLabel: String) -> String {
        ...
        var lines: [String] = []
        lines.append("Snaploom capture timing — total: \(Self.format(total))")
        lines.append("")
...
class AppDelegate: NSObject, NSApplicationDelegate, SPUUpdaterDelegate {
    ...
    /// Snaploom has been idle. Use the idle-sleep-safe variant: plain
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        let bundleID = Bundle.main.bundleIdentifier ?? "io.github.xiaoouwaou.snaploom"
        let running = NSRunningApplication.runningApplications(withBundleIdentifier: bundleID)
        if running.count > 1 {
            // Tell the existing instance to show its icon and open Settings
            DistributedNotificationCenter.default().postNotificationName(
                .init("io.github.xiaoouwaou.snaploom.showAndOpenPrefs"),
        ...
        // Disable App Nap. Snaploom is LSUIElement with no visible windows
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: .init("io.github.xiaoouwaou.snaploom.showAndOpenPrefs"), object: nil
        )
        ...
        NotificationCenter.default.addObserver(
            ...
            name: .init("Snaploom.pinFromHistory"), object: nil
        )
    ...
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        // Re-launching Snaploom while it's running: show the menu bar icon
        if UserDefaults.standard.bool(forKey: "hideMenuBarIcon") {
    ...
    private func promptToMoveToApplicationsIfNeeded() {
        ...
        do {
            ...
            errAlert.informativeText = "Please drag Snaploom to your Applications folder manually.\n\n\(error.localizedDescription)"
            errAlert.runModal()
        }
    }

    func applicationWillTerminate(_ aNotification: Notification) {
        os_log(.fault, log: timingLog, "Snaploom terminating — thermalState=%d", ProcessInfo.processInfo.thermalState.rawValue)
        for (_, controller) in overlayControllerPool {
            controller.tearDown()
        }
        overlayControllerPool.removeAll()
        HotkeyManager.shared.unregister()
        if snaploomSignalLogFd >= 0 {
            close(snaploomSignalLogFd)
            snaploomSignalLogFd = -1
        }
    }
    ...
    private func setupSignalHandlers() {
        let logDir = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
            .appendingPathComponent("Logs/Snaploom", isDirectory: true)
        try? FileManager.default.createDirectory(at: logDir, withIntermediateDirectories: true)
        let logPath = logDir.appendingPathComponent("termination.log")
        snaploomSignalLogFd = open(logPath.path, O_WRONLY | O_CREAT | O_APPEND, 0o644)
    ...
    private func setupMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "About Snaploom", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: "")
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(withTitle: "Quit Snaploom", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    private func applyPreferredIconImage(to button: NSStatusBarButton) {
        ...
        if mode == "symbol", !symbolName.isEmpty,
           let symbol = NSImage(systemSymbolName: symbolName, accessibilityDescription: "Snaploom") {
    ...
    private func rebuildStatusBarMenu() {
        ...
        let quitItem = NSMenuItem(title: L("Quit Snaploom"), action: #selector(quitApp), keyEquivalent: "q")
    ...
    /// The app that was active before Snaploom showed its overlay.
    private var previousApp: NSRunningApplication?

    /// Titled Snaploom windows (editors, preferences, Sparkle, etc.) that were
    ...
    /// Call when a Snaploom window closes. If no titled windows remain,
    ...
    /// original text on the screenshot (snaploom://ocr-translate). `target` nil
    ...
    private func showPreCaptureCountdown(seconds: Int) {
        ...
        // Local catches keys when Snaploom is active; global catches when another app has focus
    ...
    /// Real path on disk:
    ///   ~/Library/Containers/io.github.xiaoouwaou.snaploom/Data/Library/Application Support/Snaploom/timing.log
    static let timingLogURL: URL = {
        let fm = FileManager.default
        let support = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let dir = support.appendingPathComponent("Snaploom", isDirectory: true)
    ...
    /// Hide non-overlay titled Snaploom windows so they can't be dragged in
    ...
    /// trying to screenshot something *other than* Snaploom, and any Snaploom
    /// windows still on screen are unintended background clutter. When
    /// Snaploom itself is frontmost the user presumably wants to capture one
    ...
    private func stashBackgroundWindows() {
        ...
        let snaploomWasFrontmost = previousApp?.bundleIdentifier == ourBundleID
        guard !snaploomWasFrontmost else { return }
    ...
    private func openImageWithPanel() {
        ...
        panel.message = "Choose an image to open in Snaploom editor"
    ...
    /// external tools re-open a specific capture for editing — `snaploom://edit?id=<id>` —
    ...
    private func openVideoWithPanel() {
        ...
        panel.message = L("Choose a video to open in Snaploom editor")
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        ...
        for url in urls {
            if url.scheme?.lowercased() == "snaploom" {
    ...
    /// Handle snaploom:// URL scheme actions from external tools (Raycast, Alfred, etc.).
    /// Usage: `open snaploom://capture`, `open snaploom://ocr`, etc.
...
extension AppDelegate: OverlayWindowControllerDelegate {
    ...
    private func copyRecordingToClipboard(url: URL) {
        ...
        let fixedURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("snaploom-clipboard-recording.\(ext)")
    ...
    func overlayDidRequestScrollCapture(_ controller: OverlayWindowController, rect: NSRect, screen: NSScreen) {
        if !AXIsProcessTrusted() {
            ...
            alert.informativeText = L("Snaploom needs Accessibility permission for scroll capture. Please grant access in System Settings, then try again.")
    ...
    func overlayDidRequestAccessibilityPermission(_ controller: OverlayWindowController) {
        ...
        let alert = NSAlert()
        alert.messageText = L("Accessibility Access Required")
        alert.informativeText = L("Snaploom needs Accessibility permission to show keystrokes during recording. Please grant access in System Settings, then try again.")
        alert.alertStyle = .warning
        alert.addButton(withTitle: L("Open Settings"))
        alert.addButton(withTitle: L("Cancel"))
        let response = alert.runModal()
    ...
    func overlayDidRequestInputMonitoringPermission(_ controller: OverlayWindowController) {
        ...
        alert.informativeText = L("Snaploom needs Input Monitoring permission to show keystrokes during recording. Please grant access in System Settings, then try again.")
        alert.alertStyle = .warning
        alert.addButton(withTitle: L("Open Settings"))
        alert.addButton(withTitle: L("Cancel"))
        let response = alert.runModal()
    ...
    func overlayDidRequestToggleAutoScroll(_ controller: OverlayWindowController) {
        ...
        if !scc.autoScrollActive {
            if !AXIsProcessTrusted() {
                ...
                let alert = NSAlert()
                alert.messageText = L("Accessibility Access Required")
                alert.informativeText = L("Snaploom needs Accessibility permission to auto-scroll other apps. Please grant access in System Settings, then try again.")
                alert.alertStyle = .warning
                alert.addButton(withTitle: L("Open Settings"))
                alert.addButton(withTitle: L("Cancel"))
                let response = alert.runModal()
